...
* 1x AS5600（SOP8）、MT6701CT-STD（SOP8）或 TMAG5273（SOT23-6）霍尔角度传感器芯片。使用 MT6701 时，如果 PCB 上没有 JP1 跳线请剪掉它的引脚 8（Z/CSN）；使用 AS5600 时，如果 PCB 上有 JP1 跳线请用焊锡将其短接。
* RGB LED 灯
...
* 1x LP5907MFX-4.5 或 TPS7A2042PDBVR LDO（低压差稳压器），SOT23-5 封装 (U6)，注意看下一行。
* 1x LM4040 并联电压基准，请选用 2.0~2.1V (用于 4.2V LDO) 或 2.2~2.5V (用于 4.5V LDO) 基准电压的型号，SOT23-3 封装 (U5)。
...
* 7x 矮磁轴，佳达隆/NuPhy 的磁玉 Mini 或者磁玉 Pro Mini，或者凯华的星磁 Mini，或者其他兼容的品牌型号。(SW1-SW7)。
...
#### 键帽
* 键帽（"Keycap *.stl", 选择和你方案相匹配的），推荐树脂打印，但普通 FDM 打印也勉强能用。
...
### 霍尔效应版本的注意事项
* 如果你用凯华星磁轴，你需要使用 704 RTV 硅胶粘合剂将磁轴固定在 PCB 上，以防止其晃动。
* 磁轴的校准需要在命令行下进行。
...
* 打印底座和面板，给面板上的图案和文字部分分配不同的颜色。`_insert` 版本的意思是需要使用热熔螺母。建议使用透明材料。
